...
|---|---|---|---|
| A1 | **NF — nombre de factures fournisseur** | **DERIVABLE** | `VENDOR_INVOICE` is a catalogued document type (« Facture tierce payable », migration `20260714000001:58`), captured per dossier through the ordinary document flow and already an evidence key (C-3 declarable set). **NF = count of a dossier's VENDOR_INVOICE documents.** Phase-0's own source map row 61 already classified this AUTO — my earlier read.ts comment ("no source") was wrong about the platform and right only about the join I had written. Counting rule to ratify: all uploads vs `VERIFIED` only (→ Q1). Historical dossiers: countable retroactively wherever documents were attached. No migration, no new capture. |
