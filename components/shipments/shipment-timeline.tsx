...
import { buildTimeline, type ShipmentRecord } from "@/lib/shipments";
...
const nodeTone: Record<string, string> = {
  ...
  "current-exception": "border-amber-500 bg-amber-500 text-white",
...
export function ShipmentTimeline({ shipment }: { shipment: ShipmentRecord }) {
  ...
  return (
    <ol className="relative px-5 py-5">
      {steps.map((step, i) => {
        ...
        const nodeKey =
          step.state === "current" && step.tone === "amber"
            ? "current-exception"
            : step.state;
        return (
              ...
              {step.state === "current" && (
                <span
                  className={cn(
                    ...
                    step.tone === "amber" ? "bg-amber-400" : "bg-navy-500",
                  )}
                />
              )}
            </span>
