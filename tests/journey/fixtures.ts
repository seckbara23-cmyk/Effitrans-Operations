...
export async function execution(fileId: string, stepKey: string) {
  const { data } = await db()
    ...
    .select("id, state, assigned_user_id, started_at, submitted_by, submitted_at, completed_at, reviewed_by, completion_provenance, reconciled_fact, process_instance_id, evidence_summary")
    .eq("step_key", stepKey)
    .in(
      "process_instance_id",
