...
*Last updated: 2026-08-08 (**OPS-SEC-1 CLOSED — P0 anonymous RPC execution path shut; migrations 90–92 applied and verified; ONE OUTAGE during remediation from a missed transitive policy dependency, see `docs/ops/ops-sec-1-incident-report.md`. Previously: EMP-4A COMPLETE — CI GREEN run `31275167965` (#394) on `66e56d0`: rls-tests 82/0/0, build 10/0/0**.
...
| **OPS-SEC-1** (Hosted Supabase RPC exposure — P0) | 2026-08-08 **CLOSED** | `7f3f566` (CI-verified) | **90** `20260814000001` (43 fns), **91** `20260814000002` (2 fns), **92** `20260814000003` (restore 1 policy dependency) — **ALL APPLIED**, ledger repaired. Privilege-only: no function body, table, policy, trigger or row changed | CI run `31280066280` (#399) **rls-tests 82/0/0, build 10/0/0 — zero skipped, zero failed** · anonymous execution path CLOSED (external probe: 42501 before body on quotation_validate / next_quotation_number / emit_business_event) · 12-check read-only prod verification · ⚠️ **one production outage during remediation** (migration 90 revoked a TRANSITIVE policy dependency, `can_read_file` → `user_readable_file_ids`, breaking 21 policies; restored by 92) · five process failures recorded in `docs/ops/ops-sec-1-incident-report.md` |
| **EMP-4A** (Mailbox membership & user provisioning) | 2026-08-08 **CLOSED** | `66e56d0` (CI-verified head) | **89** `20260813000001_mailbox_membership.sql` — **APPLIED** in production via SQL Editor; ⚠️ **CLI ledger still needs `migration repair --status applied` for 20260812000001 + 20260813000001** (NOT db push, do NOT re-run 89). The close-out adds **no migration, permission, role, flag or schema** | gates: 213 files / 5429 tests green, tsc 0, build clean · CI run `31275167965` (#394) **rls-tests 82/0/0, build 10/0/0 — zero skipped, zero failed** (also #392 on `79457d2`) · membership NARROWS (never an alternative auth path) · preview-first enforced by a server-recomputed fingerprint that binds mailbox + capabilities + eligibility · all seven `ec_*` tables registered with the tenant-scope guard (standing gap since EC-1) |
