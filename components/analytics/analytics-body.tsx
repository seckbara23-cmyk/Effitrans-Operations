/**
 * Analytics data body (Phase 1.13/1.13B; P1: streamed). Server component.
 * ---------------------------------------------------------------------------
 * Holds the heavy aggregation (getAnalytics + getExecutiveAnalytics) + the full
 * executive render. Rendered inside <Suspense> by the page so the shell streams
 * immediately and the data-heavy content arrives when ready. No logic change —
 * same calculations, same layout.
 */
...
export async function AnalyticsBody({ canFinance }: { canFinance: boolean }) {
