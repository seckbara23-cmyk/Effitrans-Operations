...
/** Safe quick links (Phase 3.3A D12). Read-only navigation — no mutable actions. */
export function QuickActions({ fileId, contactEmail }: { fileId: string; contactEmail: string | null }) {
  ...
  const items: { label: string; href: string; icon: string }[] = [
    ...
    { label: q.contact, href: contactEmail ? `mailto:${contactEmail}` : "/portal/notifications", icon: "✉️" },
  ];
  return (
