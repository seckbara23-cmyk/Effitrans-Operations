...
import { suspendTenant, reactivateTenant, archiveTenant, type LifecycleResult } from "@/lib/platform/lifecycle-actions";
import type { RevocationSummary } from "@/lib/platform/session-revocation";
...
type Meta = {
  ...
  reversible: string;
  needsReason: boolean;
  run: (tenantId: string, reason?: string) => Promise<LifecycleResult>;
};
...
const META: Record<LifecycleAction, Meta> = {
  suspend: {
    ...
    body: "Les utilisateurs de ce tenant seront déconnectés : leurs sessions actives sont révoquées, ils ne pourront plus se connecter ni accéder à l'application, et le processus officiel sera bloqué. Les données sont conservées et restent lisibles par les administrateurs plateforme.",
  ...
  reactivate: {
    ...
    body: "L'accès des utilisateurs, l'API et le processus officiel sont rétablis. Aucune donnée n'est migrée. Les anciennes sessions ne sont pas restaurées — les utilisateurs se reconnectent.",
  ...
  archive: {
    ...
    body: "L'accès au tenant est définitivement désactivé : les sessions actives sont révoquées ; connexion, provisionnement et déploiement bloqués. Aucune donnée n'est supprimée ; elle reste consultable uniquement par les administrateurs plateforme.",
...
export function LifecycleActions({ tenantId, status }: { tenantId: string; status: LifecycleStatus }) {
  ...
  const [outcome, setOutcome] = useState<{ action: LifecycleAction; revocation?: RevocationSummary } | null>(null);
  ...
  function confirm(action: LifecycleAction) {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        // Keep the dialog open to REPORT the outcome honestly (transition applied +
        // revocation summary, including a partial failure) before the admin dismisses it.
        setOutcome({ action, revocation: res.revocation });
        setReason("");
        router.refresh();
      } else {
  ...
  function revocationLine(r: RevocationSummary): string {
    if (r.targeted === 0) return "Aucune session utilisateur à révoquer.";
    if (r.failed === 0) return `Sessions révoquées : ${r.revoked} / ${r.targeted}.`;
    return `Sessions révoquées : ${r.revoked} / ${r.targeted} · ${r.failed} échec(s) — le tenant reste suspendu (le blocage à la requête suivante s'applique déjà).`;
  }
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md rounded-xl border border-white/10 bg-navy-950 p-6">
            {outcome ? (
              // ---- the honest outcome report (transition applied + revocation summary) ----
              <div aria-live="polite">
                <h3 className="text-lg font-semibold text-white">{META[outcome.action].title} — effectué</h3>
                <p className="mt-2 text-sm text-emerald-300">Transition appliquée.</p>
                {outcome.revocation && (
                  <p className={`mt-1 text-sm ${outcome.revocation.failed > 0 ? "text-amber-300" : "text-slate-300"}`}>
                    {revocationLine(outcome.revocation)}
                  </p>
                )}
